...
public abstract class ListenerBase : IDisposable
{
    ...
    /// <summary>
    /// Base constructor of <see cref="ListenerBase"/>
    /// </summary>
    /// <param name="hooker">Depending on this parameter the listener hooks either application or global keyboard events.</param>
    /// <remarks>
    /// Hooks are not active after instantiation. You need to use either <see cref="ListenerBase.Enabled"/> property or call <see cref="ListenerBase.Start"/> method.
    /// </remarks>
    protected ListenerBase(Hooker hooker)
    {
        ArgumentNullException.ThrowIfNull(hooker);
        m_Hooker = hooker;
    }
